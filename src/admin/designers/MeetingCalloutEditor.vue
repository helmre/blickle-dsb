...
let ro = null
onMounted(() => { nowTimer = setInterval(() => { now.value = Date.now() }, 1000); recomputeScale(); if (typeof ResizeObserver !== 'undefined') { ro = new ResizeObserver(recomputeScale); if (previewFrame.value) ro.observe(previewFrame.value) } })
onUnmounted(() => { if (nowTimer) clearInterval(nowTimer); if (ro) ro.disconnect() })
</script>
...
.meta-cards { grid-column: 1 / 13; display: grid; grid-template-columns: 1fr 1fr; gap: 32px; }
.time-card { padding: 40px 48px; border-radius: 20px; background: linear-gradient(180deg, rgba(255,255,255,0.04), rgba(255,255,255,0.01)); border: 1px solid rgba(255,255,255,0.1); border-left: 6px solid var(--accent); position: relative; overflow: hidden; }
.time-icon { font-size: 48px; color: var(--accent); filter: drop-shadow(0 0 18px var(--accent)); margin-bottom: 8px; }
.time-value { font-family: var(--font-display); font-size: 180px; font-weight: 800; line-height: 0.9; letter-spacing: -0.04em; color: #fff; }
.theme-light .time-value { color: #0B1F3A; }
.time-label { font-family: var(--font-display); font-size: 24px; font-weight: 600; letter-spacing: 0.2em; text-transform: uppercase; color: rgba(255,255,255,0.55); margin-top: 4px; }
.theme-light .time-label { color: rgba(11,31,58,0.5); }
.where-card { padding: 40px 44px; border-radius: 20px; background: linear-gradient(180deg, rgba(255,255,255,0.04), rgba(255,255,255,0.01)); border: 1px solid rgba(255,255,255,0.1); display: flex; flex-direction: column; justify-content: space-between; min-height: 240px; }
.day-weekday { font-family: var(--font-display); font-size: 40px; font-weight: 700; color: var(--accent); letter-spacing: 0.04em; text-transform: uppercase; }
.day-date { margin-top: 6px; font-family: var(--font-display); font-size: 58px; font-weight: 700; color: #fff; letter-spacing: -0.02em; }
.theme-light .day-date { color: #0B1F3A; }
.where-location { display: flex; align-items: center; gap: 16px; padding-top: 20px; border-top: 1px solid rgba(255,255,255,0.1); }
.theme-light .where-location { border-top-color: rgba(0,0,0,0.12); }
.loc-icon { font-size: 42px; color: var(--accent); filter: drop-shadow(0 0 14px var(--accent)); }
.loc-name { font-family: var(--font-display); font-size: 42px; font-weight: 700; color: #fff; }
.theme-light .loc-name { color: #0B1F3A; }
.topic { grid-column: 1 / 13; font-family: var(--font-display); font-size: 70px; font-weight: 700; line-height: 1.05; letter-spacing: -0.02em; color: #fff; }
.theme-light .topic { color: #0B1F3A; }
.body-row { grid-column: 1 / 13; display: grid; grid-template-columns: 2fr 1fr; gap: 24px; align-items: center; }
.body { font-size: 30px; line-height: 1.35; color: rgba(255,255,255,0.82); }
.theme-light .body { color: rgba(11,31,58,0.78); }
.audience { text-align: right; }
.audience-tag { display: inline-block; padding: 12px 22px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.15); border-radius: 999px; font-size: 20px; font-weight: 600; color: rgba(255,255,255,0.85); }
.theme-light .audience-tag { background: rgba(11,31,58,0.06); border-color: rgba(11,31,58,0.15); color: rgba(11,31,58,0.8); }
.countdown { grid-column: 1 / 13; display: flex; align-items: center; gap: 18px; padding: 20px 32px; border-radius: 16px; background: rgba(181,204,24,0.08); border: 1px solid rgba(181,204,24,0.25); }
...
.countdown-pulse { width: 18px; height: 18px; border-radius: 50%; background: var(--accent); box-shadow: 0 0 20px var(--accent); }
.countdown.pulse .countdown-pulse { background: #EF4444; box-shadow: 0 0 24px #EF4444; }
.countdown-text { font-family: var(--font-display); font-size: 38px; font-weight: 700; color: #fff; }
.theme-light .countdown-text { color: #0B1F3A; }
.footer { grid-column: 1 / 13; display: flex; align-items: center; gap: 14px; padding-top: 16px; border-top: 1px solid rgba(255,255,255,0.08); font-size: 17px; font-weight: 500; letter-spacing: 0.05em; text-transform: uppercase; color: rgba(255,255,255,0.55); }
